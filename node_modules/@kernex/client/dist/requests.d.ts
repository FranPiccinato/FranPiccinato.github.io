 ...
 */
export declare type FieldFilter<T> = T extends any[] ? (T[number] | T) : T;
declare type QueryReservedParameters = '$limit' | '$skip' | '$select' | '$sort' | '$join';
export declare type ResourceFilters<T> = {
...
export declare type Query<T = unknown> = Partial<BaseQuery<T> & Partial<Omit<ResourceFilters<T>, QueryReservedParameters>>>;
export declare type ServerQuery<T = unknown> = Partial<Omit<Query<T>, '$join'>> & Partial<{
    $client?: {
        $join?: Query<T>['$join'];
    };
}>;
